...
ecModel = setParam(ecModel,'eq','y001714',0);
ecModel = setParam(ecModel,'lb','y001808',-1000);
...
ecModel = setParam(ecModel, 'lb', 'xBIOMASS', 0.46);
...
ecModel = setParam(ecModel,'lb','xBIOMASS',0);
ecModel = setParam(ecModel,'obj','xBIOMASS',1);
...
rxnIdx = find(strcmp(kcatList_merged.rxns,'y000079'));
...
enzMW = ecModel.ec.mw(strcmp(ecModel.ec.enzymes,'Q6BZY1')); % Get MW of the enzyme.
...
ecModel = setKcatForReactions(ecModel,'y000079',convKcat);
ecModel = applyKcatConstraints(ecModel);

saveEcModel(ecModel,'ecYaliGEM_stage3.yml');
